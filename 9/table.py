...
class Tbl():
    ...
    def add_row(self, row):
        ...
        for x in range(0, len(tokens)):
            ...
            cells.append(token)
        self.rows.append(Row(cells, self.num_columns, self.row_idx))
        self.row_idx += 1
    ...
    def add_list_row(self, row):
        if len(row) != len(self.cols):
            print("skipping row; incorrect number of columns")
            return
        self.rows.append(Row(row, self.num_columns, self.row_idx))
        self.row_idx += 1
        for x in range(0, len(self.cols)):
            if x in self.num_columns:
    ...
    def add_constructed_row(self, row):
        if len(row.cells) != len(self.cols):
            print("skipping row; incorrect number of columns")
            return
        self.rows.append(Row(row.cells,self.num_columns, self.row_idx, row.tag))
        self.row_idx += 1
        for x in range(0, len(self.cols)):
            if x in self.num_columns:
                self.cols[x] + float(row[x])
    ...
    def choose_probes(self):
        probe_ids = random.sample(range(0,len(self.rows)), 100)
        for i in range(0, len(probe_ids)):
            self.rows[probe_ids[i]].tag(i)
    ...
    def projection_split(self, initial=0):
        ...
        for col_idx in range(0, len(self.cols)):
            if col_idx in self.non_goal_columns:
                ng_cols.append(self.cols[col_idx])
        for i in range(0, 10):
            fastmap_lists.append(fastMap(self.rows, ng_cols))
        # print(fastmap_lists)
        best_l,best_r,l_pivot,r_pivot,dist = self.choose_pivots(fastmap_lists, ng_cols)
        # tmp = best_l + best_r
        # tmp.sort()
        # print(tmp)
        ...
        # make a tree node
        root = SplitNode(len(self.rows),l_pivot,r_pivot,dist)
    ...
    def choose_pivots(self, fastmap_lists, cols):
        ...
        #till here
        return best_l,best_r,self.rows[best_pivots[0]],self.rows[best_pivots[1]],pivots[2]
