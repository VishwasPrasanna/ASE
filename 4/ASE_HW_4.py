...
def main():
    ...
    # zeroR weathernon
    zr_weathernon = Tbl(weathernon_train)
    zr_weathernon_abcd = Abcd()
    # print("Zero R Weathernon\n")

    # split into lines
    rows = weathernon.splitlines()
        
    # remove empty lines
    rows.pop(0) # pops the first line, which is empty
    rows.pop() # pops the last line, which is empty# split into lines

    # add remaining rows
    for row in rows:
        zr_weathernon_abcd.update_confusion(
            'yes' if row.split(',')[4].strip() == "yes" else 'no',
            'yes' if zr_weathernon.zeroRClassify(row) == "yes" else 'no')
        # print("predicted: "+zr_weathernon.zeroRClassify(row)+", actual: "+row.split(',')[4])
        zr_weathernon.add_row(row)
    
    print("#--- zerorok -----------------------\n\nweathernon")
    zr_weathernon_abcd.calc_abcd()
    zr_weathernon_abcd.calc_perfomance()
    zr_weathernon_abcd.pretty_print()

    # zeroR diabetes
    zr_diabetes = Tbl(diabetes_train)
    zr_diabetes_abcd = Abcd()
    # print("\nZero R Diabetes\n")

    # split into lines
    rows = diabetes.splitlines()
    ...
    # add remaining rows
    for row in rows:
        zr_diabetes_abcd.update_confusion(
            'yes' if row.split(',')[8].strip() == "tested_positive" else 'no',
            'yes' if zr_diabetes.zeroRClassify(row) == "tested_positive" else 'no')
        # print("predicted: "+zr_diabetes.zeroRClassify(row)+", actual: "+row.split(',')[8])
        zr_diabetes.add_row(row)

    print("\n\ndiabetes\n")
    zr_diabetes_abcd.calc_abcd()
    zr_diabetes_abcd.calc_perfomance()
    zr_diabetes_abcd.pretty_print()

    ################################################################################

    # naiveBayes weathernon
    nb_weathernon = NaiveBayes(weathernon_train)
    nb_weathernon_abcd = Abcd()
    # print("\nNaive Bayes Weathernon\n")
    ...
    # add remaining rows
    for row in rows:
        nb_weathernon_abcd.update_confusion(
            'yes' if row.split(',')[4].strip() == "yes" else 'no',
            'yes' if nb_weathernon.predict(row) == "yes" else 'no')
        # print("predicted: "+nb_weathernon.predict(row)+", actual: "+row.split(',')[4])
        nb_weathernon + row
    
    print("#--- nbok -----------------------\n\nweathernon")
    nb_weathernon_abcd.calc_abcd()
    nb_weathernon_abcd.calc_perfomance()
    nb_weathernon_abcd.pretty_print()

    # naiveBayes diabetes
    nb_diabetes = NaiveBayes(diabetes_train)
    nb_diabetes_abcd = Abcd()
    ...
    # add remaining rows
    for row in rows:
        nb_diabetes_abcd.update_confusion(
            'yes' if row.split(',')[8].strip() == "tested_positive" else 'no',
            'yes' if nb_diabetes.predict(row) == "tested_positive" else 'no')
        # print("actual: "+row.split(',')[8]+", predicted: "+nb_diabetes.predict(row))
        nb_diabetes + row
    
    print("\n\ndiabetes\n")
    nb_diabetes_abcd.calc_abcd()
    nb_diabetes_abcd.calc_perfomance()
    nb_diabetes_abcd.pretty_print()
